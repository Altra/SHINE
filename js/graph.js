...
function updateDoppler(){
 ...
 if(objectSpeed==0){
  ...
  dcom.innerHTML = 'In front of the object the waves are compressed, so the wavelength is shorter, the frequency is higher and for sound waves it will have a higher pitch.';
   d2com.innerHTML = 'Behind the object the waves are stretched, so the wavelength is longer, the frequency is lower and it would have a lower pitch.';
 } else {
  dcom.innerHTML = 'At low speeds the effect is not as noticeable. The object is always emitting the wave at the same frequency, it just appears different to the observer';
  d2com.innerHTML = 'because the object is moving.';
 }
...
function getSpeed(slider,width){
 per = (parseInt(slider.style.left)-10)/width;
 return per * 1.7;
}
...
function startDop(){
 ...
 new Slider(document.getElementById('slider'), 10, 200, "Images/Slider");
